...
export async function GET() {
  ...
  const { data, error } = await svc.from("concursos").select("id,titulo,orgao,banca,vagas,salario,inscricao_inicio,inscricao_fim,prova_data,cargos,escolaridade,status,scope,state_code,city,latitude,longitude,location_label,hot_score,hot_reasons,edital_url,simulados(slug)").is("merged_into_id", null).gte("hot_score", 5).not("latitude", "is", null).not("longitude", "is", null).not("scope", "is", null).order("hot_score", { ascending: false }).limit(20);
