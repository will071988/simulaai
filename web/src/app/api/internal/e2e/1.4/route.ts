...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (contestBefore.id !== contestAfter.id || !secondCounts.documents.some((row) => row.relationship_type === "RETIFICATION") || contestAfter.quality_status === "CONFLICTED" || !idempotent || detail.data?.raw_text !== undefined) throw new Error("persistent E2E assertions failed");
