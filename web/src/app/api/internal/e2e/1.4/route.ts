...
export async function POST(req: Request) {
  ...
  try {
    ...
    const { data: editalAliases } = await svc.from("concurso_identity_aliases").select("concurso_id,alias_type,alias_value,source_url").eq("alias_type", "EDITAL").eq("alias_value", "01/2026").eq("is_current", true);
